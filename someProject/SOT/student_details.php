    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
        ...
        for ($i = 1; $i <= $sheetCount; $i++) {
            ...
            if (isset($spreadSheetAry[$i][7])) {
                $programCode = mysqli_real_escape_string($conn, $spreadSheetAry[$i][7]);
            }

            if (!empty($name)) {
                $query = "insert into  student_details_sot(S_roll,S_name,S_batch,S_specialization,S_email,S_semester,S_phone,S_program_code) values(?,?,?,?,?,?,?,?)";
                $paramType = "ssssssss";
                $paramArray = array(
                    $rollno,
                    $name,
...
<body>
    ...
    <div class="outer-container instructions">
        ...
        <ul>
            <li>
                <h3>Row layout:&emsp;&emsp; Roll no. , Student name , Batch , Specialization , E-mail , Semester , Phone no. , Program code</h3>
            </li>
            <li>
                <h3> Actual data should start from the second row and first column. No column and row headings </h3>
            </li>
            <li>
                <button type="input" class="btn-submit"><a style="color:white;text-decoration:none;" href="/someProject/Template/Student_details.xlsx">Download spreadsheet template</a></button>
            </li>
        </ul>
    </div>
